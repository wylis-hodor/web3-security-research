...
### [H-2] Weak randomness in `PuppyRaffle::selectWinner` allows anyone to choose winner (and in selecting the puppy)
...
3. Users can revert their `selectWinner` if the dont like the puppy.
...
2. Create a mapping of addresses -> payout so winners can pull their funds out themselves, putting the owness on the winner to claim their prize. (Recommended). Pull over Push (best practice)
...
<details>
<summary>Proof Of Code -- needed for competative audit</summary>
...
## G-1:  // slither: PuppyRaffle.raffleDuration (src/PuppyRaffle.sol#28) should be immutable
    uint256 public raffleDuration; // @audit-gas should be immutable

## G-2:  // slither: PuppyRaffle.commonImageUri (src/PuppyRaffle.sol#42) should be constant
    string private commonImageUri = "ipfs://QmSsYRx3LpDAb1GZQm7zZ1AuHZjfbPkD6J7s9r41xu1mf8";

### L-9: Costly operations inside loop (Aderyn)
...
### I-1: Unspecific Solidity Pragma (Aderyn)
...
- Found in src/PuppyRaffle.sol [Line: 2](src/PuppyRaffle.sol#L2)
    ...
    ```solidity
    pragma solidity ^0.7.6; // @audit-info floating pragma.  we want to know exact version of solidity
    ```

</details>

### I-2: // slither: solc-0.7.6 is an outdated solc version. Use a more recent version (at least 0.8.0), if possible.
// Reference: https://github.com/crytic/slither/wiki/Detector-Documentation#incorrect-versions-of-solidity

## I-3:   // @audit slither: (src/PuppyRaffle.sol#62) lacks a zero-check
        // input validation
        feeAddress = _feeAddress;

## I-4: follow CEI
    selectWinner()
    might be suseptible to re-entrancy.
...
- Found in src/PuppyRaffle.sol [Line: 161](src/PuppyRaffle.sol#L161)
    ...
    ```solidity
            uint256 prizePool = (totalAmountCollected * 80) / 100; // @audit-info do not use magic numbers; define constant
    ```

- Found in src/PuppyRaffle.sol [Line: 162](src/PuppyRaffle.sol#L162)

    ```solidity
            uint256 fee = (totalAmountCollected * 20) / 100; // q loss precision?
    ```
